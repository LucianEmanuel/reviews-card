// local reviews data
"use strict";

//Constants

const reviews = [
  {
    id: 1,
    name: "susan smith",
    job: "web developer",
    img: "https://images2.imgbox.com/e0/57/qI5bbwvg_o.jpeg",
    text: "I'm baby meggings twee health goth +1. Bicycle rights tumeric chartreuse before they sold out chambray pop-up. Shaman humblebrag pickled coloring book salvia hoodie, cold-pressed four dollar toast everyday carry",
  },
  {
    id: 2,
    name: "anna johnson",
    job: "web designer",
    img: "https://images2.imgbox.com/2e/6e/JAMvTZ56_o.jpeg",
    text: "Helvetica artisan kinfolk thundercats lumbersexual blue bottle. Disrupt glossier gastropub deep v vice franzen hell of brooklyn twee enamel pin fashion axe.photo booth jean shorts artisan narwhal.",
  },
  {
    id: 3,
    name: "peter jones",
    job: "intern",
    img: "https://images2.imgbox.com/56/88/oJvFN3l5_o.jpeg",
    text: "Sriracha literally flexitarian irony, vape marfa unicorn. Glossier tattooed 8-bit, fixie waistcoat offal activated charcoal slow-carb marfa hell of pabst raclette post-ironic jianbing swag.",
  },
  {
    id: 4,
    name: "bill anderson",
    job: "the boss",
    img: "https://images2.imgbox.com/8b/1c/vwWNTsCd_o.jpeg",
    text: "Edison bulb put a bird on it humblebrag, marfa pok pok heirloom fashion axe cray stumptown venmo actually seitan. VHS farm-to-table schlitz, edison bulb pop-up 3 wolf moon tote bag street art shabby chic. ",
  },
];

const image = document.querySelector(".image");
const author = document.querySelector(".name");
const position = document.querySelector(".position");
const description = document.querySelector(".txt");
const arrows = document.querySelectorAll(".arrow");
const randonBtn = document.querySelector(".btn");
let currentItem = 0;
let totalReviews = reviews.length - 1;

window.addEventListener("load", function () {
  let review = reviews[currentItem];
  image.src = review.img;
  author.textContent = review.name;
  position.textContent = review.job;
  description.textContent = review.text;
});

function showPerson(value) {
  let item = reviews[value];
  image.src = item.img;
  author.textContent = item.name;
  position.textContent = item.job;
  description.textContent = item.text;
}

arrows.forEach((arrow) => {
  arrow.addEventListener("click", (e) => {
    console.log(e.currentTarget);

    if (e.currentTarget.classList.contains("left")) {
      currentItem--;
      if (currentItem < 0) {
        currentItem = totalReviews;
        console.log(currentItem);
      }
      showPerson(currentItem);
    } else if (e.currentTarget.classList.contains("right")) {
      currentItem++;
      if (currentItem > totalReviews) {
        currentItem = 0;
      }
      showPerson(currentItem);
    }
  });
});
randonBtn.addEventListener("click", () => {
  let randomNum = Math.floor(Math.random() * totalReviews);
  showPerson(randomNum);
});
